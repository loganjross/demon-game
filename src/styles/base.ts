 ...
 * 
 * @property {boolean} transition (adds a transition to the component)
 * 
...
export const baseStyles = css<BaseProps>`
  ...
  border: ${({ theme, ...baseProps }) =>
    !!baseProps.border ? `solid 3px ${theme.palette[baseProps.border]}` : ""};
  border-radius: ${({ theme, ...baseProps }) =>
    !!baseProps.radius ? theme.radius * baseProps.radius + "px" : ""};
  font-size: ${({ theme, ...baseProps }) =>
    !!baseProps.fontSize ? theme.font.size[baseProps.fontSize] : ""};
  font-weight: ${({ ...baseProps }) => baseProps.fontWeight || ""};
  opacity: ${({ theme, ...baseProps }) => baseProps.opacity || ""};
  ...
  top: ${({ theme, ...baseProps }) => {
    if (!!baseProps.top) return baseProps.top + "px";
    if (!!baseProps.posCenter || !!baseProps.posCenterY) return "50%";
  }};
  right: ${({ theme, ...baseProps }) => (baseProps.right || "") + "px"};
  bottom: ${({ theme, ...baseProps }) => (baseProps.bottom || "") + "px"};
  left: ${({ theme, ...baseProps }) => {
    if (!!baseProps.left) {
      return baseProps.left + "px";
    }
